...
writes under \texttt{data/submissions/} by default, while the exploration
notebook writes run outputs under \texttt{results/}.  The script is
\texttt{src/ctmc\_submission.py}.

\section{Training Data}

The CTMC models use the checkpointed event data produced by
\texttt{src/run\_data\_pipeline.py}.  In particular, the main input is
...
\begin{verbatim}
python src/ctmc_submission.py
\end{verbatim}

The standalone script writes:
\begin{itemize}[leftmargin=*]
\item \texttt{data/submissions/ctmc\_global\_submission.csv}
\item \texttt{data/submissions/ctmc\_clustered\_submission.csv}
\item \texttt{data/submissions/ctmc\_neural\_rate\_submission.csv}
\item \texttt{data/submissions/ctmc\_blend\_submission.csv}
\end{itemize}
